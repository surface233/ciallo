...
# ========== 4. 核心：多音频播放+停止框架（修改：新增播放song.mp3的JS函数） ==========
...
# 初始化多音频播放/停止的JS（核心修改2：新增playSongAudio函数）
components.html(f"""
<script>
    // 存储所有活跃的音频实例（支持多音频叠加）
    let audioInstances = {{}};
...
with col1:
    if st.session_state.current_step1 == 1:
        ...
        if st.button("《千恋万花》，启动！"):
            ...
            # 核心修改4：替换play_audio调用为play_song_audio
...
if 0 < st.session_state.affection < 100:
    if st.session_state.current_step == 1:
        ...
        # 核心修改5：替换play_audio调用为play_song_audio
        if st.session_state.fail == 2:
            # 先停止旧的song.mp3，再播放新的
            pause_all_audio()
            play_song_audio()  # 替换原play_audio("./audio/song.mp3", custom_volume=0.1)
            st.session_state.fail = 1
    ...
    # 步骤-1：游戏结束分支（新增好感度重置）
        ...
        if st.button("第三炸弹，败者食尘！"):
            ...
            # 核心修改6：替换play_audio调用为play_song_audio
            play_song_audio()  # 替换原play_audio("./audio/song.mp3", custom_volume=0.1)
    ...
    if st.button("第三炸弹，败者食尘！"):
        ...
        stop_audio("./audio/Murasame4.mp3")
        stop_audio("./audio/man.mp3")
        # 核心修改7：替换play_audio调用为play_song_audio
        play_song_audio()  # 替换原play_audio("./audio/song.mp3", custom_volume=0.1)
        st.session_state.affection = 50  # 重置好感度
        st.session_state.current_step = 1
        st.session_state.fail = 2
